...
chicago = [41.880443, -87.644107]
art_map = folium.Map(location=[41.880443, -87.644107], zoom_start=11)


folium.Marker(location=(41.923064, -87.63504),
              popup='<b>{}</b>'.format("Francis W. Parker"),
              icon=folium.Icon(color='red', icon='graduation-cap', prefix='fa')
              ).add_to(art_map)

for i in range(len(data)):
    folium.Marker(location=(lats[i], longs[i]), popup=names[i], icon=folium.Icon(color='blue', icon='paint-brush', prefix='fa')).add_to(art_map)

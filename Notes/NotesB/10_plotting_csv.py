...
month_numbers = [x for x in range(12)]  # dummy number set, we need number against number, can't do against strings
month_names = headers[4:-1]
print(month_names)

# plt.plot(month_numbers, my_library)  # plots line graph
plt.plot(month_numbers, my_library, color='darkgreen', label='Lincoln Park')  # plots a bar graph
plt.plot(month_numbers, library2, color='blue', label='Bucktown-Wicker Park')

plt.xticks(month_numbers, month_names, rotation=45)  # replaces the shown values (dummy numbers) on the graph axis
